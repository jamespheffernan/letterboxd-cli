...
Updated 2026-05-08 after the adversarial release-readiness pass.
...
| God module | Everything lived in one large `cli.py`. | First seams extracted: `web.py` for session/HTTP safety and `output.py` for provenance/output shaping. `docs/ARCHITECTURE.md` defines next seams. |
...
- The large `cli.py` keeps shrinking. The next serious extractions are storage/migrations, parser modules, and recommendation scoring.
